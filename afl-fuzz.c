...
unsigned   number_mutations;          /* !PJ! store number of mutations   */
...
static u32 nr_splices;                /* Times this case spliced          */
...
static int limit_deterministic_stage = 0;
static u64 det_stage_limit = 0;
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    if (hnb == 2) {
      ...
      /* PJ: here we need to save and reset mutation history */
#ifndef AFL_DISABLE_MUTATION_HISTORY
      struct queue_entry *q_entry = queue_cur;
      char buffer[16 * 16], *pos=buffer;

      u32 mutation_history_saved[16];
      memcpy(mutation_history_saved, q_entry->mutation_history, 16 * sizeof(u32));
      memset(q_entry->mutation_history, 0, 16 * sizeof(u32));

      char *fnmut = (char *) alloc_printf("%s/mutation_history/id:%06u, %s", out_dir, queued_paths, describe_op(hnb));
      FILE* f = fopen(fnmut, "w");
      for (int i = 0; i < number_mutations; i++) {
	      pos += sprintf(pos, "%u,", mutation_history_saved[i]);
      }
      fputs(buffer, f);
      fclose(f);
#endif
...
static u8 fuzz_one(char** argv) {
  ...
  if (master_max && (queue_cur->exec_cksum % master_max) != master_id - 1)
	...
	// Code ported from MOpt
	cur_ms_lv = get_cur_time();
	if (limit_deterministic_stage && !((cur_ms_lv - last_path_time < det_stage_limit) ||
		(last_crash_time != 0 && cur_ms_lv - last_crash_time < det_stage_limit) || last_path_time == 0))
	{
		skip_deterministic = 1;	// skip deterministic stage from now on
		goto havoc_stage;
	}
...
#ifndef AFL_DISABLE_MUTATION_HISTORY
      queue_cur->mutation_history[mutation_chosen]++; // !PJ! increment mutation counter
#endif

      switch (mutation_chosen) {
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -L            - use MOpt-style AFL and set the limit time for entering the pacemaker fuzzing mode (set 0 will enter pacemaker mode at first). For instance, (-L 30): if MOpt-AFL finishes the mutation of one input while does not find any interesting test case for more than 30 min, MOpt-AFL will enter the pacemaker fuzzing mode.\n\n"
...
EXP_ST void setup_dirs_fds(void) {
  ...
  ck_free(tmp);
#ifndef AFL_DISABLE_MUTATION_HISTORY 
  tmp = alloc_printf("%s/mutation_history", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:L:T:dnCB:S:M:x:Qsp")) > 0)
	...
	use_splicing = 1;
  	...
  	case 'L': /* Limit time spend in deterministic stage */
	  
        if (sscanf(optarg, "%llu", &det_stage_limit) < 1 || optarg[0] == '-') {
                FATAL("Bad syntax used for -l");
        }
        else {
                limit_deterministic_stage = 1;
                SAYF("Set time limit for deterministic stage to %llu min\n", det_stage_limit);
                if (det_stage_limit > 60) {
                        SAYF("This limit is pretty high, are you sure about it?\n");
                }
                if (det_stage_limit == 0)
                        skip_deterministic = 1;
#if SKIP_SPLICING
                use_splicing = 0; /* PJ */
#else
